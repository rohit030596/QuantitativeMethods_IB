...
```{r}
lm1 = lm(sales ~ consumer_confidence + total_snow + total_rain + mean_temp + spd_max_gust, data=df)
summary(lm1)
vif(lm1)
```
e)lm2 - weather + cpi_index
```{r}
lm2 = lm(sales ~ cpi_index + total_snow + total_rain + mean_temp + spd_max_gust, data=df)
summary(lm2)
vif(lm2)
```
f)lm3 - weather + disposable_income
```{r}
lm3 = lm(sales ~ disposable_income + total_snow + total_rain + mean_temp + spd_max_gust, data=df)
summary(lm3)
vif(lm3)
```
g)lm4 - weather + unemployment_rate
```{r}
lm4 = lm(sales ~ unemployment_rate + total_snow + total_rain + mean_temp + spd_max_gust, data=df)
summary(lm4)
vif(lm4)
```
