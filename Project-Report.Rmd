...
lm.full = lm(sales ~ ., data = data_monthly[3:9])
lm.step_monthly1 = step(lm.full, direction = "backward")
summary(lm.step_monthly1)
...
data$date = str_c(data$Day,"-",data$Month,"-",data$Year)
data=data %>% relocate(date,.before=visits_by_day)
my_data=data[,6:18]
my_data$postal_code=data$postal_code
my_data <- my_data %>% replace_na(list(spd_max_gust=0))
```
#pre-holday, post holiday, day of the week, long weekend
### EDA


### Modelling
```{r}
#1. panel regression for footfall
library(plm)
base_model_random <- plm(visits_by_day ~ ConsumerConfidence + cpi + disposableIncome + unemployability, data = my_data, index = c("postal_code", "date"), model = "within")
summary(base_model_random)
```
```{r}
base_model_random <- plm(visits_by_day ~ ConsumerConfidence + cpi + disposableIncome + unemployability +as.factor(date) , data = my_data, index = c("postal_code", "date"), model = "random")
summary(base_model_random)
```

```{r}
#handling #NA
data <- data %>% replace_na(list(spd_max_gust=0))
data <- data %>% replace_na(list(snow_grnd = 0))
data <- data %>% replace_na(list(total_rain = 0))
data <- data %>% replace_na(list(total_snow = 0))
data <- data %>% replace_na(list(total_precip = 0))
data <- data %>% replace_na(list(visits_by_day = 0))

#Taking monthly data (footfall count, weather, and control variables)
my_data <- data %>% group_by(postal_code, Year, Month) %>% summarize(visits_by_day = sum(visits_by_day),
                                                                     mean_temp = mean(mean_temp),
                                                                     ConsumerConfidence = mean(ConsumerConfidence),
                                                                     cpi = mean(cpi),
                                                                     disposableIncome = mean(disposableIncome),
                                                                     unemployability = mean(unemployability),
                                                                     total_snow = sum(total_snow),
                                                                     total_rain = sum(total_rain),
                                                                     total_precip = sum(total_precip),
                                                                     snow_grnd = mean(snow_grnd),
                                                                     spd_max_gust = mean(spd_max_gust)
                                                                     )

#2. Step wise regression for monthly aggregated panel data -footfall
lm.full = lm(visits_by_day ~ ., data = my_data[,4:13])
lm.step = step(lm.full, direction = "backward")
summary(lm.step)
```

```{r}
#3. Step wise regression for monthly retail sales data - statistics Canada
test_data <- read.csv("Data/testing.csv")
lm.full = lm(value ~ ., data = test_data[2:10])
lm.step = step(lm.full, direction = "backward")
summary(lm.step)
```
```{r}
#4. plm model for the retail sales data
test_model_random <- plm(value ~ mean_temp + total_rain + total_snow, data = test_data, index = c("prov","Date"), model = "within")
summary(test_model_random)
```
```{r}
#OLS model adjusting for some multicollinear factors
test_model <- lm(value ~ max_temp + total_rain + total_snow , data=test_data)
summary(test_model)
```
### Results
